...
with tab9:
    ...
    receivables_mn = st.number_input(
        "Receivables Outstanding (₹ Mn)",
        min_value=0.0,
        value=500.0
    )
    ...
    st.info(
        f"""
        Receivables Considered : ₹{receivables_mn:,.2f} Mn
        
        Discount Rate : {discount_rate:.2f}%
        
        Borrowing Cost : {borrowing_rate:.2f}%
        
        Net Benefit Rate :
        {(borrowing_rate-discount_rate):.2f}%
        """
    )
# ======================================================
# RECOMMENDATION
